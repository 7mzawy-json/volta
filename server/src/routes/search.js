...
// TEMPORARY, removed with the listing above.
//
// Listing model names was not enough: gemini-2.5-flash appears in the list and
// still answers 404 to generateContent. So this actually TRIES the real request
// body against a few candidates and reports what came back — including whether
// dropping thinkingConfig is what makes the difference, since that field may not
// exist on the newer models.
//
// This is the measurement that picks the model. Guessing from a list is how the
// 404 happened in the first place.
searchRouter.get('/search/try', async (_req, res) => {
  if (!config.aiConfigured) return res.status(503).json({ error: 'aiNotConfigured' });

  const { systemPrompt, parseJson } = await import('../ai.js');
  const candidates = String(_req.query.models || '')
    .split(',')
    .map((m) => m.trim())
    .filter(Boolean);
  if (!candidates.length) return res.status(400).json({ error: 'noCandidates' });

  const sentence = String(_req.query.q || 'a cheap apple phone under 300 dinar');
  const results = [];
  ...
  for (const model of candidates) {
    for (const withThinking of [false, true]) {
      ...
      try {
        ...
        results.push({
          model,
          thinkingDisabled: !withThinking,
          status: upstream.status,
          ms: Date.now() - started,
          reason: body?.error?.details?.find((d) => d.reason)?.reason || body?.error?.status || null,
          finish: body?.candidates?.[0]?.finishReason ?? null,
          answered: parseJson(text)
        });
      } catch (err) {
        results.push({ model, thinkingDisabled: !withThinking, error: err?.message, ms: Date.now() - started });
      }
      // A model that worked needs no second attempt.
      if (results[results.length - 1].status === 200) break;
    }
  }

  return res.json({ sentence, results });
});
